...
/**
   ...
   - <b>Bulk I/O Service</b> Motr ioservice which process read/write FOPs.
   ...
   - <b>r.non-blocking.few-threads</b> Motr service should use a relatively
...
out:
	...
	rwrep->rwr_repair_done = m0_sns_cm_fid_repair_done(&rwfop->crw_gfid,
							   reqh, device_state);
	M0_LEAVE();
	// M0_LOG(M0_ALWAYS,"rajat [%"PRId64"] data : %02x", rwfop->crw_dummy_id, ((int *)rwfop->crw_di_data_cksum.b_addr)[0]);
	return M0_FSO_AGAIN;
}
/**
...
static int io_launch(struct m0_fom *fom)
{
	...
	m0_tl_for(netbufs, &fom_obj->fcrw_netbuf_list, nb) {
		...
		M0_ADDB2_ADD(M0_AVI_FOM_TO_STIO, fom->fo_sm_phase.sm_id,
			     stio->si_id);
		// M0_LOG(M0_ALWAYS,"rajat [%"PRId64"] parity data : %02x", stio->si_rwfop->crw_dummy_id,((int *)stio->si_rwfop->crw_di_data_cksum.b_addr)[0]);
		/*
